...
| G48 | analyze-stats | mediation-analysis guide (`analysis_guides/` or `table-types/`) — the skill has **no** mediation content despite cross-sectional mediation being one of the most-published genres: bootstrapped a×b indirect effect, proportion-mediated with uncertainty, AGReMA reporting, and the design caveat that identification (not the bootstrap) is the issue. Surfaced alongside O13 | 4 | 3 | 5 | 60 | shipped (this PR) |
| G49 | peer-review + self-review | `observational_confounding.md` **O14 — interaction scale (additive vs multiplicative)** for joint-effect/synergy/effect-modification claims: a synergy claim is an additive-scale statement needing **RERI/AP/S with CIs**, not a multiplicative-only OR product or joint-category ORs; stratified-only 'stronger in A than B' is difference-in-significance. + analyze-stats Interaction&Effect-Modification entry. Found reverse-engineering 5 CC-BY joint-effect cross-sectional papers | 4 | 4 | 4 | 64 | shipped (this PR) |
| G50 | peer-review + self-review | `observational_confounding.md` O15 candidate — exposome-/metabolome-/environment-wide multiplicity (family-wise / FDR control + selective top-hit reporting across many exposures). Partially grounded (environmental multi-exposure papers seen) but THIS round's corpus was joint-two-exposure; defer to a future exposome-wide fetch | 4 | 3 | 4 | 48 | open |
| G51 | clean-data | physiologic-range / implausible-value rule reference (e.g. SBP=0, age=999, BMI<10 or >80, negative labs) — cross-skill audit found clean-data has cleaning_patterns + structural-zero but no explicit implausible-value range-rule library. UNVERIFIED candidate — next session confirm cleaning_patterns.md does not already cover it | 3 | 4 | 3 | 36 | open |
...
- **cross-sectional cohort (cross-sectional-cohort harvest + paper-driven)** — `saturated` (review-side O1–O14 + CP1–CP4 + S9 + gates comprehensive; only exposome-wide O15 (G50) deferred): observational probes O1–O10
